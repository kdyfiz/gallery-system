 ...
 */
public class PhotoGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all photos").get("/api/photos").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new photo")
                        .post("/api/photos")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created photo").get("${new_photo_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created photo").delete("${new_photo_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Photo entity").exec(scn);
