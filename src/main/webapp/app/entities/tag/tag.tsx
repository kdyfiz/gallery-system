          ...
          <Table responsive>
            <thead>
              <tr>
                ...
                <th>
                  <Translate contentKey="gallerySystemApp.tag.albums">Albums</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="gallerySystemApp.tag.photos">Photos</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {tagList.map((tag, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {tag.albums
                      ? tag.albums.map((val, j) => (
                          <span key={j}>
                            <Link to={`/album/${val.id}`}>{val.id}</Link>
                            {j === tag.albums.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
                  <td>
                    {tag.photos
                      ? tag.photos.map((val, j) => (
                          <span key={j}>
                            <Link to={`/photo/${val.id}`}>{val.id}</Link>
                            {j === tag.photos.length - 1 ? '' : ', '}
                          </span>
                        ))
                      : null}
                  </td>
