...
public class CommentResource {
    ...
    /**
     * {@code GET  /comments/album/:albumId} : get all comments for a specific album.
     *
     * @param albumId the id of the album to get comments for.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of comments in body.
     */
    @GetMapping("/album/{albumId}")
    public ResponseEntity<List<CommentDTO>> getCommentsByAlbum(@PathVariable("albumId") Long albumId) {
        LOG.debug("REST request to get Comments for Album : {}", albumId);
        List<CommentDTO> comments = commentService.findByAlbumId(albumId);
        return ResponseEntity.ok().body(comments);
    }

    /**
     * {@code GET  /comments/photo/:photoId} : get all comments for a specific photo.
     *
     * @param photoId the id of the photo to get comments for.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of comments in body.
     */
    @GetMapping("/photo/{photoId}")
    public ResponseEntity<List<CommentDTO>> getCommentsByPhoto(@PathVariable("photoId") Long photoId) {
        LOG.debug("REST request to get Comments for Photo : {}", photoId);
        List<CommentDTO> comments = commentService.findByPhotoId(photoId);
        return ResponseEntity.ok().body(comments);
    }
